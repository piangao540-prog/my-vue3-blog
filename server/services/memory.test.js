...
describe('memory 合并逻辑', () => {
    const existing = [
        { id: 1, category: '生活状态', content: '正在准备秋招，目标是前端开发岗位', weight: 0.8, status: 'active' }
    ]

    it('相似但更新的内容 -> 更新旧记忆，不新增', () => {
        const incoming = [
            { category: '生活状态', content: '正在准备秋招，目标是前端开发岗位，已经投了几家公司', weight: 0.9 }
        ]
    ...
    it('新分类内容 -> 新增记忆', () => {
        const incoming = [
            { category: '情绪特征', content: '面试前容易紧张', weight: 0.6 }
        ]
        const { insert, update } = mergeMemories(existing, incoming)
    ...
    it('非法分类 -> 丢弃', () => {
        const incoming = [
            { category: '随便写的', content: '这条不该入库', weight: 0.5 }
        ]
        const { insert, update } = mergeMemories(existing, incoming)
        expect(insert).toHaveLength(0)
        expect(update).toHaveLength(0)
