...
export const useSearchFilter = () => {
  ...
  const filteredArticles = computed(() => {
    ...
    if (searchStore.selectedCategory) {
      articles = articles.filter((article) => article.category === searchStore.selectedCategory)
    }
    // 按标签筛选
    if (searchStore.selectedTag) {
      articles = articles.filter((article) => article.tags.includes(searchStore.selectedTag))
    }
    // 按关键词筛选（全文搜索）
